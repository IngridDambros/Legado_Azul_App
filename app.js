...
function _parcRenderizar(filtro) {
    ...
    lista.innerHTML = parceiros.map(p => `
        <div class="parcnew-card" onclick="abrirFicha('${p.id}')" style="--pc:${p.cor};--pb:${p.thumbBg}">
            <div class="parcnew-body">
                <span class="parcnew-badge">${p.subTitulo}</span>
                <div class="parcnew-nome">${p.nome}</div>
                <div class="parcnew-desc">${p.desc}</div>
                <div class="parcnew-rating">
                    <i data-lucide="star" style="width:11px;height:11px;stroke:var(--pc);fill:var(--pc);stroke-width:0"></i>
                    <span>${p.rating.toFixed(1)}</span>
                </div>
            </div>
            <div class="parcnew-right">
                <div class="parcnew-thumb">
                    <i data-lucide="${p.icone}" style="width:26px;height:26px;stroke:var(--pc);stroke-width:1.5;fill:none"></i>
                </div>
                <i data-lucide="chevron-right" style="width:15px;height:15px;stroke:rgba(30,58,71,.28);stroke-width:2.2"></i>
            </div>
        </div>
    `).join('');
    lucide.createIcons();
