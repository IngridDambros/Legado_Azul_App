...
function _abrirSeletorPilar(pilar) {
    ...
    if (pilar === 'educacao') {
        ...
        const autonomHtml = _TER_CAMPOS.filter(c => c.grupo === 'autonomia').map(c => _toggleItemHtml('terapia', c, cor)).join('');
        const comportHtml = _TER_CAMPOS.filter(c => c.grupo === 'comportamental').map(c => _toggleItemHtml('terapia', c, cor)).join('');
        const motoraHtml  = _TER_CAMPOS.filter(c => c.grupo === 'motora').map(c => _toggleItemHtml('terapia', c, cor)).join('');
        conteudo = `<div class="pil-sel-group-title">Desenvolvimento de Autonomia e Integração</div>${autonomHtml}<div class="pil-sel-group-title">Psicoterapia e Abordagens Comportamentais</div>${comportHtml}<div class="pil-sel-group-title">Terapias Motoras e Integrativas</div>${motoraHtml}`;
    } else {
        const camposArr = _PILAR_CAMPOS_MAP[pilar] || [];
    ...
    overlay.innerHTML = `<div class="pil-form-screen">
        <div class="pp2-header" style="flex-shrink:0">
            <div style="flex:1;min-width:0">
                ...
                <div style="font-family:var(--ft);font-size:.65rem;color:#7855C8;letter-spacing:.03em;margin-top:1px">Terapia</div>
            </div>
            <img src="LAbranco.png" alt="Legado Azul" class="pp2-logo" onclick="document.getElementById('pil-form-overlay').style.display='none';ir('tela-painel')" style="cursor:pointer">
        </div>
...
function renderFamilia() {
    ...
    if (!sec) return;
    sec.innerHTML = _renderPilarPanelHtml('familia', '#C84040');
    lucide.createIcons();
}
function renderSaude() {
    const sec = document.getElementById('sau-cards');
    if (!sec) return;
    sec.innerHTML = _renderPilarPanelHtml('saude', '#C8A020');
    lucide.createIcons();
}
function renderTerapia() {
    const sec = document.getElementById('ter-cards');
    if (!sec) return;
    sec.innerHTML = _renderPilarPanelHtml('terapia', '#7855C8');
    lucide.createIcons();
}
...
function _renderPilarPanelHtml(pilar, cor) {
    ...
    return `<div class="pil-cards-grid">${habilitados.map(id => {
        ...
        const dataStr = ultimo ? _dataRelativa(ultimo.data) : 'Nenhum registro';
        const sub = count > 0 ? `${count} registro${count > 1 ? 's' : ''} · ${dataStr}` : dataStr;
        const onclickFn = pilar === 'terapia' ? `_abrirFormTerapia('${id}')` : `_abrirFormPilar('${pilar}','${id}')`;
...
function renderEducacao() {
    ...
    if (!sec) return;
    sec.innerHTML = _renderPilarPanelHtml('educacao', '#3A9A6A');
    lucide.createIcons();
}
...
function _renderHojeCard() {
    ...
    const proximo = eventos.find(e => e.hora >= agora) || eventos[0];

    if (!proximo) {
        el.innerHTML = `<div class="pn-hoje" onclick="ir('tela-agenda')">
            <div class="pn-hoje-header"><span class="pn-hoje-data">${dataLabel}</span></div>
            <div class="pn-hoje-vazio">Nenhum compromisso hoje</div>
        </div>`;
        return;
    }
    const cat = _agCat(proximo.cor);
    const barCor = _corBarra[cat.cls] || '#97CBE0';
    const countLabel = eventos.length === 1 ? '1 compromisso' : `${eventos.length} compromissos`;
    el.innerHTML = `<div class="pn-hoje" onclick="ir('tela-agenda')">
        <div class="pn-hoje-header">
            <span class="pn-hoje-data">${dataLabel}</span>
            <span class="pn-hoje-count">${countLabel}</span>
        </div>
        <div class="pn-hoje-divider"></div>
        <div class="pn-hoje-evento">
            <div class="pn-hoje-barra" style="background:${barCor}"></div>
            <div>
                <div class="pn-hoje-hora">${proximo.hora}</div>
                ${proximo.fim ? `<div class="pn-hoje-ate">até ${proximo.fim}</div>` : ''}
            </div>
            <div style="flex:1;min-width:0">
                <div class="pn-hoje-nome">${proximo.texto}</div>
                <div class="pn-hoje-cat">${cat.label}</div>
            </div>
            <i data-lucide="chevron-right" style="width:16px;height:16px;stroke:rgba(255,255,255,.35);stroke-width:2;flex-shrink:0"></i>
        </div>
    </div>`;
    lucide.createIcons();
}
...
function toggleMsgFamiliaMembro(chave, chk) {
    const d = JSON.parse(localStorage.getItem('la') || '{}');
...
function _abrirSeletorMsgFamilia() {
    ...
    const presetsHtml = MSG_FAMILIA_PRESETS.map(m => {
        ...
        return `<div class="edu-toggle-item">
            <div class="edu-toggle-ico" style="background:${_hexToRgba(cor,.12)}">
                <i data-lucide="${m.icon}" style="width:16px;height:16px;stroke:${cor};stroke-width:1.7;fill:none"></i>
            </div>
            <span class="edu-toggle-nome">${m.titulo}</span>
            <label class="fe-toggle-sw" style="cursor:pointer">
                <input type="checkbox" ${on ? 'checked' : ''} onchange="toggleMsgFamiliaMembro('${m.chave}',this)">
                <span class="fe-toggle-track"></span>
            </label>
        </div>`;
    }).join('');
...
function toggleMsgEducacaoContato(chave, chk) {
    const d = JSON.parse(localStorage.getItem('la') || '{}');
...
function _abrirSeletorMsgEducacao() {
    ...
    const presetsHtml = (MSG_RESPONSAVEIS.educacao || []).map(m => {
        ...
        return `<div class="edu-toggle-item">
            ...
            <span class="edu-toggle-nome">${m.titulo}</span>
            <label class="fe-toggle-sw" style="cursor:pointer">
                <input type="checkbox" ${on ? 'checked' : ''} onchange="toggleMsgEducacaoContato('${m.chave}',this)">
                <span class="fe-toggle-track"></span>
            </label>
        </div>`;
    }).join('');
...
function toggleMsgSaudeContato(chave, chk) {
    const d = JSON.parse(localStorage.getItem('la') || '{}');
...
function _abrirSeletorMsgSaude() {
    ...
    const presetsHtml = (MSG_RESPONSAVEIS.saude || []).map(m => {
        ...
        return `<div class="edu-toggle-item">
            ...
            <span class="edu-toggle-nome">${m.titulo}</span>
            <label class="fe-toggle-sw" style="cursor:pointer">
                <input type="checkbox" ${on ? 'checked' : ''} onchange="toggleMsgSaudeContato('${m.chave}',this)">
                <span class="fe-toggle-track"></span>
            </label>
        </div>`;
    }).join('');
...
const _HUB_CFG = {
    ...
    cuid: {
        ...
        secs: [
            { icon: 'heart-handshake', tela: 'tela-cuidadores-hub' },
            { icon: 'users',           tela: 'tela-cuidadores' },
            { icon: 'handshake',       tela: 'tela-parceiros' },
            { icon: 'key',             tela: 'tela-acessos' },
        ]
    },
    traj: {
...
function _injectHubBar(screenId) {
    ...
    bar.innerHTML = hub.secs.map(s => {
        ...
        return `<button class="pil-pilar-btn${ativo ? ' sel' : ''}" ${ativo ? 'style="background:rgba(0,0,0,.12)"' : `onclick="ir('${s.tela}')"`}>
            <i data-lucide="${s.icon}" style="width:18px;height:18px;stroke:white;stroke-width:1.8;fill:none"></i>
        </button>`;
    }).join('');
...
function _renderPilarBar(barId, pilarIdx, secaoAtiva) {
    ...
    el.innerHTML = secoes.map(s => {
        ...
        return `<button class="pil-pilar-btn${ativo ? ' sel' : ''}" ${ativo ? `style="background:rgba(0,0,0,.12)"` : `onclick="${s.acao}"`}>
            <i data-lucide="${s.icon}" style="width:18px;height:18px;stroke:${p.cor};stroke-width:1.8;fill:none"></i>
        </button>`;
    }).join('');
    lucide.createIcons();
}
    ...
    const cards = responsaveis.map((r, i) => {
        ...
        <div class="msg-edu-card-wrap">
            <div class="edu-panel-card" onclick="_abrirChatResponsavel('${r.chave}','${r.titulo}','${pilarInfo?.nome}',${idx})">
                <i data-lucide="${r.icon}" class="edu-panel-bg-icon" style="width:78px;height:78px;stroke:${bc};stroke-width:1.1;fill:none;opacity:.18;pointer-events:none"></i>
                <div class="edu-panel-card-info">
    ...
    const cards = temas.map((t, i) => {
        ...
        <div class="msg-edu-card-wrap">
            <div class="edu-panel-card" onclick="abrirSalaRede('${t.chave}','${t.titulo}')">
                <i data-lucide="${t.icon}" class="edu-panel-bg-icon" style="width:78px;height:78px;stroke:${bc};stroke-width:1.1;fill:none;opacity:.18;pointer-events:none"></i>
                <div class="edu-panel-card-info">
...
function criarTelaRedeLegado() {
    ...
    tela.innerHTML = `
        <div class="pp2-header">
            <div class="com-header-info">
                <span class="pp2-nome">Rede Legado</span>
                <span class="com-canal-sub-txt" id="rede-subtitulo">Famílias que entendem de verdade</span>
            </div>
            <img src="LAbranco.png" alt="Legado Azul" class="pp2-logo">
        </div>
        ...
        <div class="pp2-bottom">
            <button class="pp2-nav-btn" onclick="fecharRedeLegado()">
                <i data-lucide="arrow-left" style="width:24px;height:24px;stroke:rgba(26,58,92,.55);stroke-width:2"></i>
            </button>
            <button class="pp2-nav-btn" onclick="ir('tela-painel')">
                <i data-lucide="arrow-right" style="width:24px;height:24px;stroke:rgba(26,58,92,.55);stroke-width:2"></i>
